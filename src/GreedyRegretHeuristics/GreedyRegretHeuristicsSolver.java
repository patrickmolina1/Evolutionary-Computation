...
public class GreedyRegretHeuristicsSolver extends Solver {
    ...
    public Solution greedy2RegretNearestNeighbor(Instance instance) {
    ...
    public Solution greedyWeightedRegretNearestNeighbor(Instance instance, double weightRegret, double weightObjective) {
        Random rand = new Random();
        List<Node> allNodes = new ArrayList<>(instance.nodes);
        Node startNode = allNodes.get(rand.nextInt(allNodes.size()));
        ...
        while (selected.size() < numToSelect && !remaining.isEmpty()) {
            ...
            for (Node candidate : remaining) {
                ...
                int regret = secondBestIncrease - bestIncrease;
                // Note: we maximize regret (higher is better) but minimize objective increase (lower is better)
